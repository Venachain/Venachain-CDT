        ...
        */
         ...
         */
            bool insert(const Key &k, const Value &v)
            {
                ...
                setState(KeyWrapper(wrapperName4_, k), total);
                total += 1;
         ...
         */
            void del(const Key &k)
            {
                ...
                getState(KeyWrapper(wrapperName4_, k), index); // 获取k->index

                bcwasm::delState(KeyWrapper(wrapperName2_, k)); // 删除 k->state
                bcwasm::delState(KeyWrapper(wrapperName4_, k)); // 删除k->index

                if (index == total - 1)
                {
                    bcwasm::delState(IndexWrapper(wrapperName1_, index)); // 删除index->k
                    setState(StrWrapper(wrapperName3_, TOTAL), --total);
                    return;
                }
                getState(IndexWrapper(wrapperName1_, total - 1), lastKey); // 获取最后一个有效的lastkey
                setState(IndexWrapper(wrapperName1_, index), lastKey);     // 更新index-> lastkey
                bcwasm::delState(IndexWrapper(wrapperName1_, total - 1));  // 删除lastindex->key
                setState(KeyWrapper(wrapperName4_, lastKey), index);       // 更新lastkey->index
         ...
         */
            ...
            const std::string wrapperName2_ = kType + Name + "key";
            const std::string wrapperName3_ = kType + Name + "string";
            const std::string wrapperName4_ = kType + Name + "ktoi"; //key to index
