...
static cl::opt<std::string> logPath(
    "log-path", cl::desc("log path, default = ./venachainabi.log"),
...
void createContractFile(fs::path &randomDir, const string &srcPath,
  ...
  while (getline(srcStream, line)) {
    if (line.find("venachain autogen end") != string::npos) {
      skip = false;
      continue;
    }
    if (line.find("venachain autogen begin") != string::npos) {
      skip = true;
      continue;
    }
    if (!skip) {
...
void prepareFile(const string &filename) {
  ...
  //    writeAnno << "/*//venachain autogen begin";
  writeAnno.close();
}
int main(int argc, const char **argv) {
  llvm::cl::SetVersionPrinter([](llvm::raw_ostream &os) {
    os << "venachain-abigen version "
       << "${VERSION_FULL}"
       << "\n";
  });
  try {
    ...
    if (verbose) {
      if (logPath.empty()) {
        auto currPath = boost::filesystem::current_path();
        logPath = std::string(currPath.string());
      }
      string logName = "venachainabi.log";
    ...
    LOGDEBUG << "the contractDef.name is " << contractDef.name;
    if (result != 0 || contractDef.name.empty()) {
      // throw Exception() << ErrStr("find macro venachain_ABI failed");
      return -1;
    }
