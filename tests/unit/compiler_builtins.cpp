#include "bcwasm/compiler_builtins.h"
...
TEST_CASE(test, ashlti3) {
  ...
  ASSERT(res == (__int128)9223372036854775808_ULLL,
...
TEST_CASE(test, ashrti3) {
  ...
  ASSERT(res == -170141183460469231731687303715884105728_LLL,
         "__ashrti3 result should be -2^127");

  __ashrti3(res, uint64_t(val), uint64_t(val >> 64), 1);
  ASSERT(res == -85070591730234615865843651857942052864_LLL,
...
TEST_CASE(test, lshlti3) {
  ...
  ASSERT((unsigned __int128)res == 2147483648_ULLL,
         "__lshlti3 result should be 2^31");

  __lshlti3(res, uint64_t(val), uint64_t(val >> 64), 63);
  ASSERT((unsigned __int128)res == 9223372036854775808_ULLL,
...
TEST_CASE(test, lshrti3) {
  ...
  ASSERT(res == (__int128)85070591730234615865843651857942052864_ULLL,
         "__lshrti3 result should be 2^126");

  __lshrti3(res, uint64_t(val), uint64_t(val >> 64), 63);
  ASSERT(res == (__int128)18446744073709551616_ULLL,
...
TEST_CASE(test, udivti3) {
  ...
  __udivti3(res, uint64_t(lhs_a), uint64_t(lhs_a >> 64), uint64_t(rhs_a),
            uint64_t(rhs_a >> 64));
  ASSERT(res == 3402823669209384634633746074317682114_ULLL,
