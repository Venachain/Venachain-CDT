...
#define DATASTREAM_CASE(NAME, TYPE, DATA)    \
  TEST_CASE(DataStream, NAME) {              \
    TYPE v = DATA;                           \
    char buf[128];                           \
    bcwasm::DataStream<char *> ds(buf, 128); \
    ds << v;                                 \
    TYPE v2;                                 \
    ds.seekp(0);                             \
    ds >> v2;                                \
...
struct StaticArray {
  ...
  BCWASM_SERIALIZE(StaticArray, (a));
};
...
DATASTREAM_CASE(map, SINGLE(std::map<int, std::string>),
...
DATASTREAM_CASE(FixedHash, SINGLE(bcwasm::FixedHash<5>),
                SINGLE({1, 2, 3, 4, 5}))
DATASTREAM_CASE(u256, bcwasm::u256, 12312312312312312)
