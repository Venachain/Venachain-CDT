...
#define ASSERT(cond, ...)                                            \
  if (!testResult_.skip) {                                           \
    if (!(cond)) {                                                   \
      ...
      bcwasm::println("assertion failed:", #cond, "line:", __LINE__, \
...
#define ASSERT_EQ(A, B, ...)                                                \
  if (!testResult_.skip) {                                                  \
    if (!((A) == (B))) {                                                    \
      ...
      testResult_.failures += 1;                                            \
      bcwasm::println("assertion failed:", #A, "==", #B, "line:", __LINE__, \
...
#define UNITTEST_MAIN()                                                    \
  ...
  int main(int argc, char *argv[]) {                                       \
    ...
    testSuit(testResult);                                                  \
    bcwasm::print(testResult.testcases, "tests,", testResult.assertions, \
