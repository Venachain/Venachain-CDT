...
#ifdef ONLY_LD
#define LD_CAT VenachainLdToolCategory
#else
#define LD_CAT VenachainCompilerToolCategory
#endif

#ifdef ONLY_LD
static llvm::cl::OptionCategory VenachainLdToolCategory("ld options");
#else
static llvm::cl::OptionCategory VenachainCompilerToolCategory("compiler options");
#endif
...
static llvm::cl::opt<std::string> logPath("log-path", 
                                    llvm::cl::desc("log path, default = <current path>"),
                                    llvm::cl::cat(LD_CAT));

static llvm::cl::opt<std::string> logLevel("log-level",
                                    llvm::cl::desc("log level, default = debug"),
                                    llvm::cl::cat(LD_CAT),
                                    llvm::cl::init("debug"));
static llvm::cl::opt<bool> verbose("verbose", llvm::cl::desc("log on-off, default = true"),
...
static llvm::cl::opt<bool> c_opt(
    ...
    llvm::cl::cat(VenachainCompilerToolCategory));
static llvm::cl::list<std::string> I_opt(
    "I", llvm::cl::desc("Add directory to include search path"),
    llvm::cl::cat(VenachainCompilerToolCategory), llvm::cl::Prefix,
    llvm::cl::ZeroOrMore);
...
static Options CreateOptions() {
  ...
  if (abigen_opt) {
    ...
    opts.abigen_opts.emplace_back("-extra-arg=-I" + venachain::cdt::utils::pwd());
    opts.abigen_opts.emplace_back(
        "-extra-arg=-I" + venachain::cdt::utils::where() + "/../include/libcxx");
    opts.abigen_opts.emplace_back(
        "-extra-arg=-I" + venachain::cdt::utils::where() + "/../include/libc");
    opts.abigen_opts.emplace_back("-extra-arg=-I" +
                                  venachain::cdt::utils::where() + "/../include");
  }
...
#ifdef ONLY_LD
  opts.ld_opts.emplace_back("-L" + venachain::cdt::utils::where() + "/../lib");
#endif

  opts.compiler_opts.emplace_back("-I " + venachain::cdt::utils::pwd());
  opts.compiler_opts.emplace_back("-I " + venachain::cdt::utils::where() +
                                  "/../include/libcxx");
  opts.compiler_opts.emplace_back("-I " + venachain::cdt::utils::where() +
                                  "/../include/libc");
  opts.compiler_opts.emplace_back("-I " + venachain::cdt::utils::where() +
                                  "/../include");
  opts.compiler_opts.emplace_back("--sysroot=" + venachain::cdt::utils::where() +
                                  "/../");
  opts.ld_opts.emplace_back("-L " + venachain::cdt::utils::where() + "/../lib");
  opts.compiler_opts.emplace_back("-c");
  ...
  if (o_opt.empty()) {
    ...
    opts.ld_opts.emplace_back("-o");
    opts.ld_opts.emplace_back(o_opt); 
