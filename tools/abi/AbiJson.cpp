...
namespace venachain {
    ...
    static const string convertBuildinType(const string &typeName,const string &akaTypeName) {
        ...
        else if (akaTypeName == "int128_s") buildInType = "int128_s";
        else if (akaTypeName == "uint128_s") buildInType = "uint128_s";
        else if (akaTypeName == "int256_s") buildInType = "int256_s";
        else if (akaTypeName == "uint256_s") buildInType = "uint256_s";
        ...
        else if (typeName == "long double") buildInType = "float128";
...
//                writer.String(abiDef.abis[i].types[j].typeName);
                writer.Key("type");
                writer.String(convertBuildinType(abiDef.abis[i].types[j].realTypeName,abiDef.abis[i].types[j].typeName));
                writer.EndObject();
            }
            writer.EndArray();
            writer.Key("outputs");
            ...
            if (abiDef.abis[i].returnType.realTypeName != "void") {
                ...
                writer.String("");
                writer.Key("type");
                writer.String(convertBuildinType(abiDef.abis[i].returnType.realTypeName,abiDef.abis[i].returnType.typeName));
                writer.EndObject();
            }
            writer.EndArray();
            writer.Key("constant");
...
//        writer.StartArray();
        for (size_t i = 0; i < contractDef.eventDef.events.size(); i++) {
            ...
            for (size_t j = 0; j < contractDef.eventDef.events[i].args.size(); j++) {
                writer.StartObject();
                writer.Key("type");
                writer.String(convertBuildinType(contractDef.eventDef.events[i].args[j],contractDef.eventDef.events[i].args[j]));
                writer.EndObject();
            }
            writer.EndArray();
            writer.Key("type");
